...
#' 
#' 
#' # lista_cli is a list of data frames of the type "series", each one referring to one station - see function "climate".
#' # clima_81_10 is a list of data frames having climatic means of temperature and precipitation, each one referring to one station. It can be the output of function "climate".
#' library(geosphere) # required for function daylength
#' thornt_lst<-NULL
#' for(k in 1 : length(lista_cli))
#'   thornt_lst[[k]]<-thornthwaite(series=lista_cli[[k]], clim_norm=clima_81_10[[k]],latitude = 46, first.yr=1981, last.yr=2010, snow_melt_coeff=c(0.5,0.5 )  )
