NULL
#' @description Plots 12-month quantile climate charts from output of function \code{\link{thornthwaite}} (Thornthwaite and Mather's water balance).
#' 
#' @param q_list a list of quantile data frames of water balance variables to be plotted, as output of function \code{\link{thornthwaite}}.
#' @param save_dir name of destination directory for graphs (if any). Default is NULL.
...
#' @param title logic. If TRUE inserts titles in charts.
...
#' 
#' 
#' # quantiles is the list of quantile tables generated by function thornthwaite; it is the second element of the output list, which can be split into two separate lists (see function thornthwaite)
#' sta <- 1     # 1st station in the list of quantile tables
#' plot_quant(q_list=quantiles[[sta]], st_name=names(quantiles)[sta], variables=c("Precipitation",  "Et0"), leg_pos = "topleft", col=c(1:6,1), pch=c(1:6,16),  lty=1, horiz=TRUE,  y.intersp=0.1)
#' 
#' @seealso \code{\link{thornthwaite}}
#'   


plot_quant<-function(q_list, save_dir=NULL, format=NULL, variables=c("Precipitation","Et0","Storage","Prec. - Evap.","Deficit","Surplus"), title=TRUE, trace_grid=TRUE, st_name=NULL, u_y_scale_magn=0.2, l_y_scale_magn=0, leg_pos="topleft", ...)
{
